...
for toks in (x.strip().split("\t") for x in open(sys.argv[1])):
    ...
    if toks[1] in '"{}"'.format([65,129]):
        print('\t'.join([str(toks[9].count('A')), str(toks[9].count('T')), str(toks[9].count('G')), str(toks[9].count('C')), str(toks[9].count('N')), str(len(toks[9]))]), file = outfile)
    elif toks[1] in '"{}"'.format([99,97,89,83,81,163,161,147,145,113]):
        print('\t'.join([str(toks[9].count('T')), str(toks[9].count('A')), str(toks[9].count('C')), str(toks[9].count('G')), str(toks[9].count('N')), str(len(toks[9]))]), file = outfile)
    else:
        error_count += 1
