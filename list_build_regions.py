...
for chrom in set(genes.chrom):
    ...
    for i, row in chrom_genes.iterrows():
        ...
        if max_idx is None:
            ...
            #1/0
            print '\t'.join([row.chrom, str(min_position['bp']), str(row.end), row.gene, row.pathway])
            continue
        ...
        print '\t'.join([row.chrom, str(min_position['bp']), str(max_position['bp']), row.gene, row.pathway])
