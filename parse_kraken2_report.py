...
parser = argparse.ArgumentParser(
    description="parse a kraken2 report, applying filters, and summarizing"
)
parser.add_argument(
    "-r",
    "--min-reads",
    ...
    type=int,
    default=0,
    help="minimum number of reads required to include",
)
parser.add_argument(
    "-t",
    "--taxon",
    dest="taxon",
    type=list,
    default=["U", "R", "D", "k", "P", "C", "O", "F", "G", "S"],
    help="taxon levels to include",
)
...
with open(args.out, "w") as out, open(args.report) as report:
    ...
    # read in report
    for (percent_total, reads, taxon_reads, taxon, NCBI_taxon_ID, name) in (
        l.strip().split("\t") for l in report
    ):
