...
parser.add_argument(
    "-n", "--sample_size", dest="n", help="number of individals in each group"
)
parser.add_argument("-o", "--result", dest="o", help="name of file contaning results")
parser.add_arugment(
    "-p", "--prs", dest="prs", help="PRS or other column to generate groups"
)
parser.add_argument(
    "-q",
    "--quartile",
    dest="q",
    help='quartile (or other such -ile), either "lower" or "upper"',
)
